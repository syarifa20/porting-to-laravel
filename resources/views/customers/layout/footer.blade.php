<script>
    ...
    $(document).ready(function() {
        ...
        $("#tree").jqGrid({
            url: "/mdgrid", // The server-side URL to fetch data from
            datatype: "local",
            data: {!! json_encode($customers) !!},
            mtype: "GET", // The format of data being fetched
        ...
        $('#tree-2').jqGrid('setGridParam', {
            url: 'show.php?id_customer=' + idCustomer
        }).trigger('reloadGrid')
    }
        ...
        caption: "Tambah",
         ...
         onClickButton: function() {
                // Membuat modal
                $(customerDialog).load('form/create.php',function(){
        ...
        function exportDialog(){
            ...
            buttons: {
                ...
                click: function() {
                    ...
                    let url = `report/export.php?${params}&selected=${selected}&id_customer=${id_customer}`;
                        if (postData.filters) {
                ...
                buttons: {
                    'Report':  {
                        ...
                        click: function() {
                            ...
                            let url = `report/report.php?${params}&start=${start}&limit=${limit}`;
                                if (postData.filters) {
                               url += `&filters=${postData.filters}`;
                                }
                                if (postData.global_search) {
    ...
    function hapusDialog(){
        ...
        var rowData = $('#tree').jqGrid('getRowData', selectedRow);
        var id_customer = rowData.id_customer; 
        $(customerDialog).load('form/hapus.php?id_customer='+id_customer ,function(){
        }).dialog({
            modal: true,
            title: "Hapus Data",
            width:900, // Mengatur lebar dialog menjadi 1000px
    ...
    function editDialog(){
                var selectedRow = $('#tree').jqGrid('getGridParam', 'selrow');
                var rowData = $('#tree').jqGrid('getRowData', selectedRow);
                var id_customer = rowData.id_customer; 1
                $(customerDialog).load('form/edit.php?id_customer='+id_customer, function() {
                    }).
                    dialog({
                    modal: true,
                    title: "Edit Data",
